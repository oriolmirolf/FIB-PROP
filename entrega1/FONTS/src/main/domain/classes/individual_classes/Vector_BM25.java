...
public class Vector_BM25 extends Vector_EV {
    ...
    protected double calcular_coordenades(String paraula) {
		Double coord = 0.0;
		if (aparicions_paraula.containsKey(paraula)) coord = (((double)aparicions_paraula.get(paraula)*(k+1.0))/((double)paraules_totals_document)+k*(1.0-b+b*((double)paraules_totals_document/((double)paraules_totals_n_documents/(double)n_documents))))*(Math.log(1+(((double)n_documents-(double)n_documents_contenen_paraula.get(paraula)+0.5)/(((double)n_documents_contenen_paraula.get(paraula)+0.5)))));
		return coord;
    }
