...
public class IndexEspaiVectorial implements Serializable{
    private Map<String, ArrayList<Vector_EV>> index;
	private Map<String, Set<String>> documents_contenen_paraula;
	private Map<String, ArrayList<ArrayList<String>>> documents_mes_semblants_a_document;
    ...
    public IndexEspaiVectorial() {
		...
		documents_mes_semblants_a_document = new TreeMap<String, ArrayList<ArrayList<String>>>();
    }
    ...
    private void recalcularVectors() {
	 ...
	 /**
	  * Mètode per recalcular les similaritats entre els Documents
	  */
	private void recalcularDocumentsSemblants() {
		documents_mes_semblants_a_document.clear();
		for (Map.Entry<String, ArrayList<Vector_EV>> entry : index.entrySet()) {
			Map<String, Double> m0 = new HashMap<String, Double>();
			Map<String, Double> m1 = new HashMap<String, Double>();
			String path = entry.getKey();
			for (Map.Entry<String, ArrayList<Vector_EV>> entry2 : index.entrySet()) {
				String path_d = entry2.getKey();
				if (path != path_d) {
					m0.put(path_d, compararDocuments(path, path_d, 0));
					m1.put(path_d, compararDocuments(path, path_d, 1));
				}
			}
			Map<String, Double> sorted0 = sortByValue(m0);
			Map<String, Double> sorted1 = sortByValue(m1);
			ArrayList<String> res0 = new ArrayList<String>();
			ArrayList<String> res1 = new ArrayList<String>();
			for (Map.Entry<String, Double> entry3 : sorted0.entrySet()) res0.add(entry3.getKey());
			for (Map.Entry<String, Double> entry3 : sorted1.entrySet()) res1.add(entry3.getKey());
			ArrayList<ArrayList<String>> f = new ArrayList<ArrayList<String>>();
			f.add(res0);
			f.add(res1);
			documents_mes_semblants_a_document.put(path, f);
		}
	}
	 ...
	 * @throws DocumentJaExisteix Excepció que es llença quan el document ja existeix.
	 * @throws FormatNoReconegut Excepció que es llença quan s'intenta afegir un document amb format no reconegut.
	 */
    public void afegirDocument(String path, ArrayList<String> paraules) throws DocumentJaExisteix, FormatNoReconegut {
		...
		this.recalcularDocumentsSemblants();
	 ...
	 * @throws NoExisteixDocument Excepció que es llença quan s'intenta
	 * @throws FormatNoReconegut Excepció que es llença quan s'intenta eliminar un document amb format no reconegut.
	 */
	 ...
	 * @throws FormatNoReconegut Excepció que es llença quan s'intenta modificar un document amb format no reconegut.
	 */
	 ...
	 * @param op Sistema de representació que agafem per fer aquesta comparació (0 = TFIDF, 1 = BM25)
	 * @throws NumDocumentsIncorrecte Excepció que es llença quan s'introdueix un número de documents incorrectes.
	 * @throws FormatNoReconegut Excepció que es llença quan s'intenta cercar un document amb format no reconegut.
	 * @throws NoExisteixDocument
	 */
    public ArrayList<String> llistarKDocumentsSemblants(int k, String path, int op) throws NumDocumentsIncorrecte, NoExisteixDocument, FormatNoReconegut {
		if (!extensioCorrecta(path)) throw new FormatNoReconegut();
		if (k < 0) throw new NumDocumentsIncorrecte();
		if (!index.containsKey(path)) throw new NoExisteixDocument();
		ArrayList<String> res = new ArrayList<String>();
		ArrayList<ArrayList<String>> llista = documents_mes_semblants_a_document.get(path);
		int value= Math.min(k, llista.get(0).size());
		for (int i = 0; i < value; ++i) res.add(llista.get(op).get(i));
		return res;
    }
	 ...
	 * @param op Sistema de representació que agafem per fer aquesta comparació (0 = TFIDF, 1 = BM25)
	 * @return Retorna una llista amb la informació dels documents rellevants.
	 * @throws NumDocumentsIncorrecte Excepció que es llença quan s'introdueix un número de documents incorrectes.
	 */
